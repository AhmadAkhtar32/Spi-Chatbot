"""
Small resilience layer around Mistral calls — mirrors gemini_utils.py.
"""
...
def _is_transient(exc: Exception) -> bool:
    msg = str(exc).lower()
    return "429" in msg or "503" in msg or "rate limit" in msg or "capacity" in msg


def generate_with_retry(client, max_retries: int = 3, **kwargs):
    """client.chat.complete(**kwargs), retried on rate-limit/overload errors."""
    delay = 2
    for attempt in range(max_retries):
        try:
            return client.chat.complete(**kwargs)
        except Exception as exc:
            logger.warning("chat.complete failed (attempt %d/%d): %s", attempt + 1, max_retries, exc)
...
def embed_with_retry(client, max_retries: int = 3, **kwargs):
    """client.embeddings.create(**kwargs), retried on rate-limit/overload errors."""
    delay = 2
    for attempt in range(max_retries):
        try:
            return client.embeddings.create(**kwargs)
        except Exception as exc:
            logger.warning("embeddings.create failed (attempt %d/%d): %s", attempt + 1, max_retries, exc)
            if not _is_transient(exc) or attempt == max_retries - 1:
                raise
            time.sleep(delay)
            delay *= 2
...
def friendly_error_message(exc: Exception) -> str:
    ...
    if "503" in msg or "capacity" in msg:
